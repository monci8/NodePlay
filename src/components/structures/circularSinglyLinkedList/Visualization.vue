...
import MethodBar from "@/components/structures/circularSinglyLinkedList/MethodBar.vue";
import { useVisualization } from "@/composables/useVisualization";
import CircularSinglyLinkedListClass from "@/utils/circularSinglyLinkedList";
...
// Defining method handlers for managing the circular singly linked list operations
const methodHandlers = (structure: any, isInit: any, dialogVisible: any, output: any) => ({
  initStructure: () => {
    structure.value.initStructure();
    isInit.value = structure.value.isInitialized();
  },
...
setupStructure(structure, CircularSinglyLinkedListClass, output, dialogVisible, isAnimating, statusInfo);
</script>

<template>
  <BaseVisualization
      :structureName="$t('structures.circularSinglyList')"
